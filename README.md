...
Manual GitHub Actions workflows are available for each state LGD code in `states.csv`. Each workflow runs the first four pipeline steps for one state, converts the resulting GeoJSONL into PMTiles and GeoParquet, compresses the `Projects_<STATE>.csv` and `Projects_<STATE>.geojsonl` files with 7-Zip, and uploads those archives plus PMTiles/GeoParquet release assets and a per-run metadata text file to the current month’s immutable GitHub release (`datasets-YYYY-MM`). During release publishing, any PMTiles or GeoParquet output that reaches GitHub’s 2 GB asset limit is partitioned before upload: PMTiles become `Projects_<STATE>-part*.pmtiles` with a companion `Projects_<STATE>.mosaic.json`, and GeoParquet becomes `Projects_<STATE>.*.parquet` with a companion `Projects_<STATE>.parquet.meta.json`. The newest month release is marked as GitHub’s Latest release.
